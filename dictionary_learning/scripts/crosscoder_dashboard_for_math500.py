...
DEFAULT_OUTPUT_FILE_PATH = "/data/analysis/openthoughts-110k-50000/v3/dashboard_data_5000.txt"
DEFAULT_MODEL_TYPE = "crosscoder" # "crosscoder" 或 "batchtopkcrosscoder"
DEFAULT_MAX_LENGTH = 5000 # Tokenizer的最大长度

# nnsight的tracer参数
tracer_kwargs = {"scan": False, "validate": False}

def get_target_latent_indices(crosscoder_model, relative_norm_threshold=0.9, top_k=10):
    """计算相对解码器范数，并返回高于阈值的潜在激活的索引。"""
    with torch.no_grad():
        ...
        # 返回同时满足两个条件的潜在激活
        final_indices = target_indices[torch.isin(target_indices, top_k_indices)]
...
def analyze_prompt_activations(
    prompt_text: str,
    base_llm: LanguageModel,
    instruct_llm: LanguageModel,
    ...
    # 1. 从Base LLM获取激活
    base_model_acts = get_activations_from_llm(prompt_text, base_llm, tokenizer, layer_idx, device, max_length)
    
    # 2. 从Instruct LLM获取激活
    instruct_model_acts = get_activations_from_llm(prompt_text, instruct_llm, tokenizer, layer_idx, device, max_length)

    # 确保两个模型的 token 数量一致 (在padding和过滤后应该一致)
    num_tokens = min(base_model_acts.shape[0], instruct_model_acts.shape[0])
    if base_model_acts.shape[0] != instruct_model_acts.shape[0]:
        print(f"警告: Token数量不匹配: Base LLM {base_model_acts.shape[0]}, Instruct LLM {instruct_model_acts.shape[0]}. 取最小的: {num_tokens}")
    
    base_model_acts = base_model_acts[:num_tokens]
    instruct_model_acts = instruct_model_acts[:num_tokens]

    if num_tokens == 0:
        print("警告: 处理后没有token，跳过此提示。")
        return []

    # 3. 准备CrossCoder的输入
    # CrossCoder期望输入形状为 (batch_size_tokens, num_models, activation_dim)
    # 这里 batch_size_tokens 是实际的token数量
    stacked_acts = torch.stack([base_model_acts, instruct_model_acts], dim=1).to(device).float()
    print(f"CrossCoder的输入形状: {stacked_acts.shape}")

    # 4. 通过CrossCoder获取潜在激活
    with torch.no_grad():
        # crosscoder_model.encode 的输出形状是 (num_tokens, dict_size)
        latent_acts_all = crosscoder_model.encode(stacked_acts)
    ...
    if isinstance(token_ids_for_prompt, int): # 如果只有一个token
        token_ids_for_prompt = [token_ids_for_prompt]
    token_ids_for_prompt = token_ids_for_prompt[:num_tokens] # 确保长度一致
